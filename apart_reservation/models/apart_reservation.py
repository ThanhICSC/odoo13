...
class apartFolio(models.Model):
    ...
    _inherit = 'apart.folio'
    _order = 'reservation_id desc'

    reservation_id = fields.Many2one('apart.reservation',
    ...
    def write(self, vals):
        ...
        res = super(apartFolio, self).write(vals)
        reservation_line_obj = self.env['apart.flat.reservation.line']
                ...
                for reservation in folio_obj.reservation_id:
                    ...
                    if len(reservation_obj) == 1:
                        for line_id in reservation.reservation_line:
                            ...
                            for flat_id in line_id:
                                vals = {'flat_id': flat_id.id,
...
class apartFolioLineExt(models.Model):

    _inherit = 'apart.folio.line'

    @api.onchange('checkin_date', 'checkout_date')
    def on_change_checkout(self):
        res = super(apartFolioLineExt, self).on_change_checkout()
        apart_flat_obj = self.env['apart.flat']
        avail_prod_ids = []
        apart_flat_ids = apart_flat_obj.search([])
        for flat in apart_flat_ids:
            assigned = False
            for line in flat.flat_reservation_line_ids:
    ...
    def write(self, vals):
        ...
        Update apart flat Reservation line history"""
        reservation_line_obj = self.env['apart.flat.reservation.line']
        flat_obj = self.env['apart.flat']
        ...
        if prod_id and is_reserved:
            ...
            prod_flat = flat_obj.search(prod_domain, limit=1)
            if (self.product_id and self.checkin_date and self.checkout_date):
                old_prd_domain = [('product_id', '=', self.product_id.id)]
                old_prod_flat = flat_obj.search(old_prd_domain, limit=1)
                if prod_flat and old_prod_flat:
                    # Check for existing flat lines.
                    srch_rmline = [('flat_id', '=', old_prod_flat.id),
                                   ('check_in', '=', self.checkin_date),
                                   ('check_out', '=', self.checkout_date),
                                   ]
                    rm_lines = reservation_line_obj.search(srch_rmline)
                    if rm_lines:
                        rm_line_vals = {'flat_id': prod_flat.id,
                                        'check_in': chkin,
                                        'check_out': chkout}
                        rm_lines.write(rm_line_vals)
        return super(apartFolioLineExt, self).write(vals)


class apartReservation(models.Model):

    _name = "apart.reservation"
    ...
    warehouse_id = fields.Many2one('stock.warehouse', 'apart', readonly=True,
                                   index=True,
    ...
    reservation_line = fields.One2many('apart_reservation.line', 'line_id',
                                       'Reservation Line',
                                       help='apart flat reservation details.',
                                       readonly=True,
                                       states={'draft': [('readonly', False)]},
                                       )
    ...
    folio_id = fields.Many2many('apart.folio', 'apart_folio_reservation_rel',
    ...
    def unlink(self):
        ...
        return super(apartReservation, self).unlink()

    
    def copy(self):
        ctx = dict(self._context) or {}
        ctx.update({'duplicate': True})
        return super(apartReservation, self.with_context(ctx)).copy()

    @api.constrains('reservation_line', 'adults', 'children')
    def check_reservation_flats(self):
        '''
            ...
            for rec in reservation.reservation_line:
                if len(rec.reserve) == 0:
                    raise ValidationError(_(
                        'Please Select flats For Reservation.'))
                for flat in rec.reserve:
                    cap += flat.capacity
            if not ctx.get('duplicate'):
                if (reservation.adults + reservation.children) > cap:
                    raise ValidationError(_(
                        'flat Capacity Exceeded \n'
                        ' Please Select flats According to'
    ...
    def onchange_partner_id(self):
        ...
        partner_shipping_id and pricelist_id of the apart reservation as well
        ---------------------------------------------------------------------
        @param self: object pointer
        '''
    ...
    def create(self, vals):
        ...
        vals['reservation_no'] = self.env['ir.sequence'].\
            next_by_code('apart.reservation') or 'New'
        return super(apartReservation, self).create(vals)
    ...
    def confirmed_reservation(self):
        """
        This method create a new record set for apart flat reservation line
        -------------------------------------------------------------------
        @param self: The object pointer
        @return: new record set for apart flat reservation line.
        """
        reservation_line_obj = self.env['apart.flat.reservation.line']
        vals = {}
        for reservation in self:
            ...
            flat_bool = False
            for line_id in reservation.reservation_line:
                for flat_id in line_id.reserve:
                    if flat_id.flat_reservation_line_ids:
                        for reserv in flat_id.flat_reservation_line_ids.\
    ...
    def cancel_reservation(self):
        """
        This method cancel record set for apart flat reservation line
        ------------------------------------------------------------------
        @param self: The object pointer
        @return: cancel record set for apart flat reservation line.
        """
        flat_res_line_obj = self.env['apart.flat.reservation.line']
        apart_res_line_obj = self.env['apart_reservation.line']
        self.state = 'cancel'
        flat_reservation_line = flat_res_line_obj.search([('reservation_id',
                                                           'in', self.ids)])
        flat_reservation_line.write({'state': 'unassigned'})
        flat_reservation_line.unlink()
        reservation_lines = apart_res_line_obj.search([('line_id',
                                                        'in', self.ids)])
        for reservation_line in reservation_lines:
            reservation_line.reserve.write({'isflat': True,
                                            'status': 'available'})
        return True
    ...
    def action_send_reservation_mail(self):
        ...
        try:
            template_id = (ir_model_data.get_object_reference
                           ('apart_reservation',
                            'email_template_apart_reservation')[1])
        except ValueError:
            template_id = False
        try:
        ...
        ctx = {
            'default_model': 'apart.reservation',
    ...
    def create_folio(self):
        """
        This method is for create new apart folio.
        -----------------------------------------
        @param self: The object pointer
        @return: new record set for apart folio.
        """
        apart_folio_obj = self.env['apart.folio']
        flat_obj = self.env['apart.flat']
        for reservation in self:
            folio_lines = []
            ...
            for line in reservation.reservation_line:
                for r in line.reserve:
                    folio_lines.append((0, 0, {
                        ...
                        'is_reserved': True}))
                    res_obj = flat_obj.browse([r.id])
                    res_obj.write({'status': 'occupied', 'isflat': False})
            folio_vals.update({'flat_lines': folio_lines})
            folio = apart_folio_obj.create(folio_vals)
            if folio:
                for rm_line in folio.flat_lines:
                    rm_line.product_id_change()
            self._cr.execute('insert into apart_folio_reservation_rel'
...
class apartReservationLine(models.Model):

    _name = "apart_reservation.line"
    _description = "Reservation Line"

    name = fields.Char('Name')
    line_id = fields.Many2one('apart.reservation')
    reserve = fields.Many2many('apart.flat',
                               'apart_reservation_line_flat_rel',
                               'apart_reservation_line_id', 'flat_id',
                               domain="[('isflat','=',True),\
                               ('categ_id','=',categ_id)]")
    categ_id = fields.Many2one('apart.flat.type', 'flat Type')
    ...
    def on_change_categ(self):
        ...
        '''
        apart_flat_obj = self.env['apart.flat']
        apart_flat_ids = apart_flat_obj.search([('categ_id', '=',
                                                 self.categ_id.id)])
        flat_ids = []
        if not self.line_id.checkin:
            raise ValidationError(_('Before choosing a flat,\n You have to \
    ...
    def unlink(self):
        ...
        @return: True/False.
        """
        apart_flat_reserv_line_obj = self.env['apart.flat.reservation.line']
        for reserv_rec in self:
            for rec in reserv_rec.reserve:
                hres_arg = [('flat_id', '=', rec.id),
                            ('reservation_id', '=', reserv_rec.line_id.id)]
                myobj = apart_flat_reserv_line_obj.search(hres_arg)
                if myobj.ids:
                    rec.write({'isflat': True, 'status': 'available'})
                    myobj.unlink()
        return super(apartReservationLine, self).unlink()


class apartflatReservationLine(models.Model):

    _name = 'apart.flat.reservation.line'
    _description = 'apart flat Reservation'
    _rec_name = 'flat_id'

    flat_id = fields.Many2one('apart.flat', string='flat id')
    check_in = fields.Datetime('Check In Date', required=True)
    check_out = fields.Datetime('Check Out Date', required=True)
    state = fields.Selection([('assigned', 'Assigned'),
                              ('unassigned', 'Unassigned')], 'flat Status')
    reservation_id = fields.Many2one('apart.reservation',
                                     string='Reservation')
...
class apartflat(models.Model):
    ...
    flat_reservation_line_ids = fields.One2many('apart.flat.reservation.line',
                                                'flat_id',
                                                string='flat Reserve Line')
    ...
    def unlink(self):
        ...
        """
        for flat in self:
            for reserv_line in flat.flat_reservation_line_ids:
                if reserv_line.status == 'confirm':
                    raise ValidationError(_('User is not able to delete the \
                                            flat after the flat in %s state \
                                            in reservation')
                                          % (reserv_line.status))
        return super(apartflat, self).unlink()

    @api.model
    def cron_flat_line(self):
        """
        This method is for scheduler
        every 1min scheduler will call this method and check Status of
        flat is occupied or available
        --------------------------------------------------------------
        @param self: The object pointer
        @return: update status of apart flat reservation line
        """
        reservation_line_obj = self.env['apart.flat.reservation.line']
        folio_flat_line_obj = self.env['folio.flat.line']
        now = datetime.now()
        curr_date = now.strftime(dt)
        for flat in self.search([]):
            reserv_line_ids = [reservation_line.id for
                               reservation_line in
                               flat.flat_reservation_line_ids]
            reserv_args = [('id', 'in', reserv_line_ids),
                           ('check_in', '<=', curr_date),
                           ('check_out', '>=', curr_date)]
            reservation_line_ids = reservation_line_obj.search(reserv_args)
            flats_ids = [flat_line.ids for flat_line in flat.flat_line_ids]
            rom_args = [('id', 'in', flats_ids),
                        ('check_in', '<=', curr_date),
                        ('check_out', '>=', curr_date)]
            flat_line_ids = folio_flat_line_obj.search(rom_args)
            status = {'isflat': True, 'color': 5}
            if reservation_line_ids.ids:
                status = {'isflat': False, 'color': 2}
            flat.write(status)
            if flat_line_ids.ids:
                status = {'isflat': False, 'color': 2}
            flat.write(status)
            if reservation_line_ids.ids and flat_line_ids.ids:
                raise ValidationError(_('Please Check flats Status \
                                         for %s.' % (flat.name)))
        return True


class flatReservationSummary(models.Model):

    _name = 'flat.reservation.summary'
    _description = 'flat reservation summary'
    ...
    flat_summary = fields.Text('flat Summary')
    ...
    def default_get(self, fields):
        ...
        if self._context is None:
            self._context = {}
        res = super(flatReservationSummary, self).default_get(fields)
    ...
    def flat_reservation(self):
        ...
        model_data_ids = mod_obj.search([('model', '=', 'ir.ui.view'),
                                         ('name', '=',
                                          'view_apart_reservation_form')])
        resource_id = model_data_ids.read(fields=['res_id'])[0]['res_id']
        return {'name': _('Reconcile Write-Off'),
                'context': self._context,
                'view_type': 'form',
                'view_mode': 'form',
                'res_model': 'apart.reservation',
    ...
    def get_flat_summary(self):
        ...
        if self.date_from and self.date_to:
            ...
            flat_ids = flat_obj.search([])
            all_flat_detail = []
            for flat in flat_ids:
                flat_detail = {}
                flat_list_stats = []
                flat_detail.update({'name': flat.name or ''})
                if not flat.flat_reservation_line_ids and \
                   not flat.flat_line_ids:
                    for chk_date in date_range_list:
                        flat_list_stats.append({'state': 'Free',
                                                'date': chk_date,
                                                'flat_id': flat.id})
                else:
                    for chk_date in date_range_list:
                        ...
                        reserline_ids = flat.flat_reservation_line_ids.ids
                        ...
                        if not reservline_ids:
                            ...
                            reservline_ids = (reservation_line_obj.search
                                              ([('id', 'in', reserline_ids),
                                                ...
                                                ('check_out', '>=', chk_date),
                                                ('state', '=', 'assigned')]))
                            for res_flat in reservline_ids:
                                cid = res_flat.check_in
                                cod = res_flat.check_out
                                dur = cod - cid
                                if flat_list_stats:
                                    count = 0
                                    for rlist in flat_list_stats:
                                        cidst = datetime.strftime(cid, dt)
                                        codst = datetime.strftime(cod, dt)
                                        rm_id = res_flat.flat_id.id
                                        ci = rlist.get('date') >= cidst
                                        co = rlist.get('date') <= codst
                                        rm = rlist.get('flat_id') == rm_id
...
#                                        extra minutes and give the flat
...
#                                        free
                                        ...
                                        reservline_ids = False
                        fol_flat_line_ids = flat.flat_line_ids.ids
                        chk_state = ['draft', 'cancel']
                        folio_resrv_ids = (folio_flat_line_obj.search
                                           ([('id', 'in', fol_flat_line_ids),
                                             ('check_in', '<=', chk_date),
                                             ('check_out', '>=', chk_date),
                                             ('status', 'not in', chk_state)
                                             ]))
                        if reservline_ids or folio_resrv_ids:
                            flat_list_stats.append({'state': 'Reserved',
                                                    'date': chk_date,
                                                    'flat_id': flat.id,
                                                    'is_draft': 'No',
                                                    'data_model': '',
                ...
                flat_detail.update({'value': flat_list_stats})
                all_flat_detail.append(flat_detail)
            main_header.append({'header': summary_header_list})
            self.summary_header = str(main_header)
            self.flat_summary = str(all_flat_detail)
        return res


class QuickflatReservation(models.TransientModel):
    _name = 'quick.flat.reservation'
    _description = 'Quick flat Reservation'

    partner_id = fields.Many2one('res.partner', string="Customer",
                                 required=True)
    check_in = fields.Datetime('Check In', required=True)
    check_out = fields.Datetime('Check Out', required=True)
    flat_id = fields.Many2one('apart.flat', 'flat', required=True)
    warehouse_id = fields.Many2one('stock.warehouse', 'apart', required=True)
    ...
    def default_get(self, fields):
        ...
        if self._context is None:
            self._context = {}
        res = super(QuickflatReservation, self).default_get(fields)
        if self._context:
            keys = self._context.keys()
            if 'date' in keys:
                res.update({'check_in': self._context['date']})
            if 'flat_id' in keys:
                flatid = self._context['flat_id']
                res.update({'flat_id': int(flatid)})
        return res

    
    def flat_reserve(self):
        """
        This method create a new record for apart.reservation
        -----------------------------------------------------
        @param self: The object pointer
        @return: new record set for apart reservation.
        """
        apart_res_obj = self.env['apart.reservation']
        for res in self:
            rec = (apart_res_obj.create
